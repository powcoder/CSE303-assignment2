...
cse303.do_cmd("Setting key b1.", "OK", client.kvI(alice, "b1", fileb1), server)
for i in range(4096):
    key = "k"+str(i)
    file = key + ".dat"
    cse303.build_file(file, 64)
    cse303.do_cmd("Setting key "+key+".", "OK", client.kvI(alice, key, file), server)
cse303.do_cmd("Persisting", "OK", client.persist(alice), server)
...
for i in range(4096):
    key = "k"+str(i)
    file = key + ".dat"
    cse303.do_cmd("Getting key "+key+".", "OK", client.kvG(alice, key), server)
    cse303.check_file_result(file, key)
    cse303.delfile(file)
cse303.do_cmd("Getting key b1.", "OK", client.kvG(alice, "b1"), server)
