...
cse303.do_cmd("Setting alice's content.", "OK", client.setC(alice, afile1), server)
cse303.do_cmd("Checking alice's content.", "OK", client.getC(alice, alice.name), server)
cse303.check_file_result(afile1, alice.name)
cse303.do_cmd("Getting all users to make sure it's just alice.", "OK", client.getA(alice, allfile), server)
...
cse303.do_cmd("Checking alice's old content.", "OK", client.getC(alice, alice.name), server)
cse303.check_file_result(afile1, alice.name)
...
cse303.do_cmd("Attempting to access alice's data by bob.", "OK", client.getC(bob, alice.name), server)
cse303.check_file_result(afile1, alice.name)
cse303.do_cmd("Getting bob's nonexistent data.", "ERR_NO_DATA", client.getC(bob, bob.name), server)
cse303.do_cmd("Getting all users to make sure it's alice and bob.", "OK", client.getA(alice, allfile), server)
...
server.pid = cse303.do_cmd_a("Restarting server:", [
    ...
    "Loaded: " + server.dirfile], server.launchcmd())
cse303.waitfor(2)
cse303.do_cmd("Setting alice's content.", "OK", client.setC(alice, afile2), server)
cse303.do_cmd("Checking alice's content.", "OK", client.getC(alice, alice.name), server)
